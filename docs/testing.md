...
- Enable locally with `RUN_SUPABASE_INTEGRATION=1 pnpm vitest run __tests__/integration --pool=forks --maxWorkers=1 --no-file-parallelism` (CI sets this flag so the suite runs on every pipeline). The GitHub Actions workflow installs the Supabase CLI, caches its binaries plus downloaded Postgres images to speed up `supabase start`, and fans the job into the required status checks alongside lint/unit runs.
- The main `ci.yml` workflow gates merges by running the messaging integration spec to keep turnaround times reasonable, while the scheduled `supabase-integration.yml` workflow (or manual dispatch) runs the full integration suite overnight or on demand for broader coverage.
